...
2. Create a backup of the database:
	```bash
	docker exec immich-db /bin/bash -c '/usr/bin/pg_dump -Fc -U postgres immich' > ${DOCKER_VOLUME}/immich/db/backup.dump
...
3. At this point, move the mounted directory to some place else. For example:
	```bash
	mv ${DOCKER_VOLUME}/immich/db ${DOCKER_VOLUME}/immich/db-pg14
	```
...
6. Once the database is up and ready to accept connections (you can check this by viewing the logs), go ahead and copy the `backup.dump` file into the new volume mount, and import the database from the backup you created in step 1.
	```bash
	cp ${DOCKER_VOLUME}/immich/db-pg14/backup.dump ${DOCKER_VOLUME}/immich/db/
	docker exec immich-db pg_restore --clean -U postgres -d immich /var/lib/postgresql/data/backup.dump
	```
	
7. You're now running the latest supported version of PostgreSQL with Immich.